...
class UI(ParentView):
    ...
    def _init_ui(self):
        ...
        # Column headers
        self.tree.heading("#0", text="Select", anchor=tk.W)
    ...
    def _insert_result_row(self, row):
        ...
        lot_number = row.get("lot_number", "")          # nuovo
        level = row.get("level", None)                  # nuovo
        level_display = f"{level}" if level is not None else ""  # es. L1, L2
        ...
        # *** nuovo ordine valori, con Batch e Level ***
        values = (
            test_method_display,
            lot_number,
            level_display,
