...
import ui.login  # For Change User feature
...
import ui.notes
import ui.result
import ui.export_notes
import ui.counts
import ui.plots
import ui.set_zscore
import ui.observations
import ui.youden
import ui.youden_selector
import ui.users
import ui.samples
import ui.analitycal_goals
import ui.tea
import ui.analytical
...
class Main(tk.Toplevel):
    ...
    def init_menu(self):
        ...
        m_main.add_cascade(label="QC", underline=0, menu=m_plots)
        ...
        if is_admin:
            items.append(("Insert random results", 0, self.on_insert_demo_result))
        items.extend([
        ...
        if is_admin:
            m_file.add_command(label="Change Laboratory",
                               underline=7,
                               accelerator="Ctrl+E",
        ...
        # === QC MENU ===
        items = (("Levey-Jennings", 0, self.on_plots),
                 ("Youden", 0, self.on_youden),
                 ("Tea", 0, self.on_tea),
                 ("Bland-Altman", 0, self.on_bland_altman),
                 ("Performance Dashboard", 0, self.on_performance_dashboard),)
        ...
        if m_edit:
            items = [
                ("Batches", 0, self.on_batches),              # B / L(otti)
                ("Categories", 0, self.on_categories),        # C / C(ategorie)
                ("Test Methods", 5, self.on_test_methods),    # L(ab) / L(aboratorio)
                ("Workstations", 0, self.on_workstations),    # W / P(ostazioni)
            ]
            # Settings only for lab admins+
            if is_lab_admin:
                items.extend([
                    ("Set Observations", 4, self.on_observations),   # O / O(sservazioni)
                    ("Set Z Score", 4, self.on_set_zscore),          # Z / Z
                ])
        ...
        if m_adm:
            ...
            items = (("Actions", 0, self.on_actions, ROLE_APP_ADMIN),
                     ("Controls", 0, self.on_controls, ROLE_APP_ADMIN),
                     ("Equipments", 0, self.on_equipments, ROLE_APP_ADMIN),
                     ("Methods", 0, self.on_methods, ROLE_APP_ADMIN),
                     ("Samples", 0, self.on_samples, ROLE_APP_ADMIN),
                     ("Suppliers", 0, self.on_suppliers, ROLE_APP_ADMIN),
                     ("Tests", 0, self.on_tests, ROLE_APP_ADMIN),
                     ("Units", 0, self.on_units, ROLE_APP_ADMIN),
                     ("Users", 0, self.on_users, ROLE_LAB_ADMIN),)
    ...
    def _build_ui(self):
        ...
        self.frmBatches = ttk.LabelFrame(frm_lists, text="Batches")
        w = self.frmBatches
        ...
        self.lstBatches.heading("level", text="Liv", anchor=tk.W)
        ...
        frm_batch = ttk.LabelFrame(frm_stats, text="Batch", labelanchor="n", width=75)
        frm_batch.pack(side=tk.LEFT, fill=tk.Y, expand=0, padx=(0, 2))

        ttk.Label(frm_batch, text="Target", anchor=tk.CENTER, width=8).pack(fill=tk.X)
        ttk.Label(frm_batch,
                  style="Target.TLabel",
                  anchor=tk.CENTER,
                  width=8,
                  textvariable=self.target).pack(fill=tk.X, padx=4, pady=1)
        ttk.Label(frm_batch, text="SD", anchor=tk.CENTER, width=8).pack(fill=tk.X)
        ttk.Label(frm_batch,
                  style="black_and_white.TLabel",
                  anchor=tk.CENTER,
                  width=8,
                  textvariable=self.sd).pack(fill=tk.X, padx=4, pady=1)
        ttk.Label(frm_batch, text="TE%", anchor=tk.CENTER, width=8).pack(fill=tk.X)
        ttk.Label(frm_batch,
                  style="black_and_white.TLabel",
                  anchor=tk.CENTER,
                  width=8,
        ...
        frm_calc = ttk.LabelFrame(frm_stats, text="Computed", labelanchor="n", width=75)
        frm_calc.pack(side=tk.LEFT, fill=tk.Y, expand=0, padx=2)

        ttk.Label(frm_calc, text="Mean", anchor=tk.CENTER, width=8).pack(fill=tk.X)
        ttk.Label(frm_calc,
                  style="Average.TLabel",
                  anchor=tk.CENTER,
                  width=8,
                  textvariable=self.average).pack(fill=tk.X, padx=4, pady=1)
        ttk.Label(frm_calc, text="sd", anchor=tk.CENTER, width=8).pack(fill=tk.X)
        ttk.Label(frm_calc,
                  style="black_and_white.TLabel",
                  anchor=tk.CENTER,
                  width=8,
                  textvariable=self.calculated_sd).pack(fill=tk.X, padx=4, pady=1)
        ttk.Label(frm_calc, text="CV%", anchor=tk.CENTER, width=8).pack(fill=tk.X)
        ttk.Label(frm_calc,
                  style="black_and_white.TLabel",
                  anchor=tk.CENTER,
                  width=8,
        ...
        frm_qc = ttk.LabelFrame(frm_stats, text="QC", labelanchor="n", width=75)
        frm_qc.pack(side=tk.LEFT, fill=tk.Y, expand=0, padx=(2, 0))

        ttk.Label(frm_qc, text="Bias%", anchor=tk.CENTER, width=8).pack(fill=tk.X)
        ttk.Label(frm_qc,
                  style="black_and_white.TLabel",
                  anchor=tk.CENTER,
                  width=8,
                  textvariable=self.bias).pack(fill=tk.X, padx=4, pady=1)
        ttk.Label(frm_qc, text="U", anchor=tk.CENTER, width=8).pack(fill=tk.X)
        ttk.Label(frm_qc,
                  style="black_and_white.TLabel",
                  anchor=tk.CENTER,
                  width=8,
                  textvariable=self.uncertainty).pack(fill=tk.X, padx=4, pady=1)
        ttk.Label(frm_qc, text="Westgard", anchor=tk.CENTER, width=8).pack(fill=tk.X)
        ...
        self.lstResults.heading("result", text="Value", anchor=tk.CENTER)
    ...
    def init_status_bar(self):
        ...
        ttk.Label(frm_status_bar, text="Lab:").pack(side=tk.RIGHT, fill=tk.X)
    ...
    def _populate_batches(self):
        ...
        self.frmBatches.config(text=f"Batches ({count})")
    ...
    def on_test_methods(self):
        ...
        ui.test_methods.UI(self).on_open()
    ...
    def on_workstations(self,):
        ...
        ui.workstations.UI(self).on_open()
    ...
    def on_observations(self,):
        ui.observations.UI(self).on_open()
        
    def on_analitical(self,):
        ui.analytical.UI(self).on_open()
    ...
    def _on_youden_plot(self, test_method, workstation, batches, data):
        ...
        """
        ui.youden.UI(self).on_open(test_method, workstation, batches, data)

    def on_bland_altman(self):
        """Open Bland-Altman comparison view."""
        ui.bland_altman.UI(self).on_open()
    ...
    #     ui.quick_data_analysis.UI(self).on_open()
    ...
    def on_change_user(self, _evt=None):
        ...
        try:
            ...
            # Show login dialog
            login_window = ui.login.Login(self)
    ...
    def _apply_section_change(self, new_section_id, sections):
        ...
        if section_data:
            ...
            display_name = f"Section {new_section_id}"

        messagebox.showinfo("Section Changed", f"Now working in: {display_name}", parent=self)
