...
From says which computer it was done at. It is worth a column because the
database file can live on a shared folder, and then a change can come from
any of the benches that reach it - and because when an account has been
shared, which it should not be and is, it is the only thing left that tells
two people apart.
...
COLUMNS = (("#0", "id", tk.W, False, 0, 0),
           ...
           ("#9", "By whom", tk.W, False, 90, 110),
           ("#10", "From", tk.W, True, 90, 115))
...
class UI(Window, tk.Toplevel):
    ...
    def put(self, row, when, what, who, where, tags):
    ...
    def get_rows(self):
        ...
        sql = """SELECT a.operation, a.result, a.received, a.reagent_lot,
                        a.status, a.log_time, a.log_host,
    ...
    def get_who(self, name):
        """A name, a machine, or nothing at all.
