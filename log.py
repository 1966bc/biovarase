...
handlers, formats. This class does only what Biovarase needs, so that a reader
can see what a logger does underneath: take the time, find out who is calling,
append one entry to a file, and move the file aside when it is full.

It also keeps the trace: with `python3 biovarase.py --trace`, trace() prints on
the terminal, line by line, what the program is doing and what its variables
hold, while the window is in use.
"""
...
class Log:
    ...
    def trace(self, message):
        ...
        watching the program work, beside its window. Each line says when, who
        - the class and method that called trace() - and what.

        It prints the data too: the values of a form, a row, the arguments of a
        statement. In a laboratory that means control results and lot numbers
        on the terminal of whoever started the program, so it is for looking at
        the program with the sample database, not for a shift at the bench.

        inspect.currentframe() is the frame running this method; f_back is the
        one that called it, and its local 'self' is the object at work.
        """
        if self.tracing:
    ...
    def rotate(self):
        ...
        becomes .2, the file becomes .1, and the next entry starts a new file.
        os.replace overwrites its target, so the oldest copy goes.
        """
