...
an application needs is one open window per name: one list of lots, one chart
per test, one dialog at a time for a result. So this is a register of the open
windows, by name - a pattern also called Multiton.

Two rules:

    show(name, build)     a list, a chart, About: if it is already open, bring
                          it to the front; build it only if it is not.
...
class Windows:
    ...
    def get(self, name):
        """The window open under this name, or None."""
        return self.dict_instances.get(name)
    ...
    def add(self, name, build):
        ...
        window.on_open()
        self.log.trace("{0}: built; dict_instances = {1}".format(
            name, list(self.dict_instances)))
    ...
    def forget(self, name, window, evt):
        ...
        A Toplevel receives <Destroy> for every widget inside it as well: only
        its own counts. And only while it is still the window registered under
        that name, not one that has already been replaced.
        """
        if str(evt.widget) == str(window) and self.dict_instances.get(name) is window:
            del self.dict_instances[name]
            self.log.trace("{0}: forgotten; dict_instances = {1}".format(
                name, list(self.dict_instances)))
