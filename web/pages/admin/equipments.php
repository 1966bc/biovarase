...
/**
 * CRUD for Equipments
 * Global table (App Admin only) - Instrument manufacturers/models
 */

$pageTitle = 'Equipments - Biovarase';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($action === 'create') {
            ...
            $stmt = $pdo->prepare("INSERT INTO equipments (supplier_id, description, status) VALUES (?, ?, 1)");
            $stmt->execute([$supplier_id ?: 0, $description]);
            $message = t('Equipment') . " '$description' " . t('created successfully');
            $messageType = 'success';
            ...
            $stmt = $pdo->prepare("UPDATE equipments SET supplier_id = ?, description = ?, status = ? WHERE equipment_id = ?");
            $stmt->execute([$supplier_id ?: 0, $description, $status, $id]);
            $message = t('Equipment') . ' ' . t('updated successfully');
            $messageType = 'success';

        }
    } catch (PDOException $e) {
        if (strpos($e->getMessage(), 'Duplicate entry') !== false) {
            $message = t('An equipment with this description already exists');
        } else {
...
try {
    ...
    // Build query
    $sql = "SELECT e.equipment_id, e.supplier_id, e.description, e.status,
    ...
    if (!empty($search)) {
        $sql .= " AND (e.description LIKE ? OR s.description LIKE ?)";
        $params[] = "%$search%";
        $params[] = "%$search%";
    }
    if (!$showDisabled) {
    ...
    $sql .= " ORDER BY s.description, e.description";
    ...
    // Counts
    $stmt = $pdo->query("SELECT COUNT(*) FROM equipments WHERE status = 1");
    $activeCount = $stmt->fetchColumn();
    $stmt = $pdo->query("SELECT COUNT(*) FROM equipments WHERE status = 0");
...
<div class="admin-container">
    <div class="breadcrumb">
        <a href="/biovarase/admin"><?= t('Admin') ?></a> / <?= t('Equipments') ?>
    </div>

    <div class="admin-header">
        <h1><?= t('Equipments') ?></h1>
        <button class="btn btn-success" onclick="openCreateModal()">+ <?= t('New') ?> <?= t('Equipment') ?></button>
    </div>
...
<div class="modal-overlay" id="editModal">
    <div class="modal">
        <h2 id="modalTitle"><?= t('New') ?> <?= t('Equipment') ?></h2>
        <form method="post" id="editForm">
            <input type="hidden" name="action" id="formAction" value="create">
            <input type="hidden" name="equipment_id" id="formPk" value="">
...
function openCreateModal() {
    document.getElementById('modalTitle').textContent = '<?= t('New') ?> <?= t('Equipment') ?>';
...
function openEditModal(row) {
    document.getElementById('modalTitle').textContent = '<?= t('Edit') ?> <?= t('Equipment') ?>';
    document.getElementById('formAction').value = 'update';
    document.getElementById('formPk').value = row.equipment_id;
